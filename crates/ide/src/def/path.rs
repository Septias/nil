...
impl Path {
    pub(crate) fn resolve_path_query(db: &dyn Database, path: Path) -> Option<VfsPath> {
    ...
    pub fn data(self, db: &dyn Database) -> PathData {
        db.lookup_intern_path(self)
    }

    pub fn resolve(self, db: &dyn Database) -> Option<VfsPath> {
